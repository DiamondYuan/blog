...
hexo.extend.tag.register('stream', function(args, content){
  ...
  result += content;
  result += '</div>';
  result += '<script src="https://cdn.staticfile.org/jquery/3.2.1/jquery.min.js" type="text/javascript">$(\'img.hexo-image-steam-lazy\').lazyload({ effect:\'fadeIn\' });</script>';
  return result;
}, {ends: true});
...
hexo.extend.tag.register('figure', function(args){
  ...
  result += '<figcaption>' + hexo.render.renderSync({text: title, engine: 'markdown'}).replace(/<p>/, '<center>').replace(/<.p>/, '</center>') + '</figcaption>';
  result += '</figure>';
  return result;
});
